...
public class BattleshipGame {
    ...
    private boolean gameOver = false; // New flag to stop the game
    ...
    private boolean hardMode = false; // Flag for hard mode
    ...
    public void placeShip(int x, int y) {
        if (gameOver) return; // Prevent placing ships if the game is over
        ...
        if (playerBoard.placeShip(currentShip, x, y, horizontal)) {
            ...
            if (currentShipIndex >= ships.length) {
                placingShips = false;
                startGame(); // Automatically start the game after placing ships
            }
        } else {
    ...
    private void computerTurn() {
        ...
        if (lastHitX != -1 && lastHitY != -1) {
            // Shoot neighboring field if in grid
        ...
        // Random shot when nothing found
        if (x == -1 && y == -1) {
            do {
